-- ============================================================================
-- 0042: Arreglar "metadata" ambiguo en client_create_reservation + archivo comprador
-- ============================================================================
-- 1) En INSERT...SELECT desde service_options, la PL/pgSQL variable "metadata"
--    chocaba con la columna so.metadata → error "column reference metadata is ambiguous".
--    (También afectaba flujos del admin si el RPC fallaba en cadena.)
-- 2) buyer_archived_at: el comprador puede ocultar reservas terminadas de "Mis reservas"
--    sin borrar el registro (solo UPDATE del propio buyer).
-- ============================================================================
...
COMMENT ON COLUMN public.reservations.buyer_archived_at IS
  'When set by the buyer, hidden from "My reservations" list; row remains for staff/history.';
...
CREATE OR REPLACE FUNCTION public.public_catalog_active_option_count()
RETURNS integer
LANGUAGE sql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.public_catalog_active_option_count() TO anon, authenticated;
