-- ============================================================================
-- 0045: Capacity check per availability window (not whole calendar day)
-- ============================================================================
-- Before: used_spots summed all reservations on the same local date, so a
-- booking at 10:00 consumed capacity for 15:00 on the same day.
-- After: only reservations whose local time falls in the same [start_time,end_time]
-- window as the new booking count toward that slot's capacity.
-- ============================================================================
...
BEGIN
  ...
  IF v_scheduled_for IS NOT NULL THEN
    ...
    SELECT a.capacity, a.start_time, a.end_time
    INTO cap, slot_start, slot_end
    FROM public.service_option_availability a
    WHERE a.service_option_id = option_id
      AND a.weekday = dow
      AND local_t >= a.start_time
    ...
    WHERE r.service_option_id = option_id
      ...
      AND EXTRACT(
        DOW
        FROM timezone('America/Costa_Rica', r.scheduled_for)
      )::integer = dow
      AND (timezone('America/Costa_Rica', r.scheduled_for))::time >= slot_start
      AND (timezone('America/Costa_Rica', r.scheduled_for))::time <= slot_end;
